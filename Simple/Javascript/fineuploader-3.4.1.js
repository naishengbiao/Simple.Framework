...
qq.FineUploaderBasic = function(o){
    ...
    this._options = {
        ...
        messages: {
            typeError: "{file} 错误的文件类型,正确类型: [{extensions}]",
            sizeError: "{file} 文件大小超出上传限制,可上传大小: {sizeLimit}",
            minSizeError: "{file} 文件大小必须大于 {minSizeLimit}",
            emptyError: "{file} 请选择文件",
            noFilesError: "没有文件上传",
            tooManyItemsError: "文件数量过多 ({netItems}), 一次只能上传 {itemLimit}个文件",
            retryFailTooManyItems: "重试失败",
            onLeave: "文件正在上传,你需要取消吗"
        },
        retry: {
            enableAuto: false,
            maxAutoAttempts: 3,
