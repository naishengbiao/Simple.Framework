(function ($) {
    $.fn.PullBox = function (options) {
        var defaults = {
            ...
            Isscreen: false,
            //创建选择框DIV
            rect: $("<div id='rect' class='pullbox-rect'></div>")
        };
    ...
    var down = function (event, options) {
        options.windowidth = $(parent.document).find("#content").width();
        options.windowheight = $(parent.document).find("#content").height();
        // 鼠标按下时才允许处理鼠标的移动事仿
        select = true;
        // 取得鼠标按下时的坐标位置
        ...
        for (var k = 1; k < tr.length; k++) {
            ...
            if (sr && item.className.indexOf(options.selectClass) == -1) {
                ...
                for (var j = 0; j < ck.length; j++) {
                    if (ck[j].type == "checkbox")
                        ck[j].checked = "checked";
                }
